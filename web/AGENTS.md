...
- Underline and arrow selections must expose draggable handles at both endpoints. Moving either endpoint changes the annotation's length and direction within the canvas, keeps the opposite endpoint fixed, and records the whole gesture as one undo step.
- Ellipse annotations remain axis-aligned: their horizontal and vertical diameters are straight, with no decorative rotation or skew in preview or MP4 export.
...
- Hand and pen are one visual overlay whose project-level size is adjustable from 0–150% in 5% steps (default 100%). At 0%, the overlay is hidden while annotations continue rendering. The value must persist across reloads, update preview immediately, produce the same MP4 geometry, and invalidate stale renders when changed.
...
- The hand overlay is compact: show the complete writing hand and only a short section immediately below the wrist, never a long forearm. Keep the pen fully visible with transparent breathing room around both ends.
